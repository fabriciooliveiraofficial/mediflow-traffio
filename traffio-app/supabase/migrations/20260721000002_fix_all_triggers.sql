-- Definitive Fix for CRM and NPS Triggers
-- 1. Fixes the OLD.status bug in crm_trg_appointments that crashed INSERTs.
-- 2. Restores the appointment_created CRM event logic.
-- 3. Fixes the v_resolved assignment syntax error in enqueue_nps_on_completion.

BEGIN;

-- =============================================================================
-- FIX 1: enqueue_nps_on_completion
-- Issue: v_resolved := public.resolve_notification_channel(...) is illegal 
-- because it returns a TABLE. Must use SELECT * INTO.
-- =============================================================================
...
BEGIN
  ...
  -- FIX: Use SELECT * INTO for table-returning function
...
-- =============================================================================
-- FIX 2: crm_trg_appointments
-- Issue: OLD is unassigned on INSERT. Trigger evaluated OLD.status at root level.
-- Also restored the appointment_created log event logic that was deleted.
-- =============================================================================
...
BEGIN
  ...
  -- Corrigido para converter corretamente date + time no fuso horario do tenant
  ...
  -- ==========================================
  -- FIX: Handle INSERT correctly (OLD is NULL)
  -- ==========================================

-- Migration: Fix clamp_to_send_window function call in enqueue_nps_on_completion
-- Problem: enqueue_nps_on_completion() calls public.clamp_to_send_window(uuid, timestamptz),
-- but the function was defined as public.crm_clamp_to_send_window(uuid, timestamptz).
-- This wrapper function ensures public.clamp_to_send_window(...) resolves correctly.

BEGIN;

CREATE OR REPLACE FUNCTION public.clamp_to_send_window(p_tenant_id uuid, p_ts timestamptz)
RETURNS timestamptz
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT public.crm_clamp_to_send_window(p_tenant_id, p_ts);
$$;

-- Também atualiza a função enqueue_nps_on_completion() para usar public.crm_clamp_to_send_window diretamente

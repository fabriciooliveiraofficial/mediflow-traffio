  ...
  try {
    ...
    for (const tenant of tenants) {
      const { data: pending, error: pendingErr } = await supabase
        ...
        .select("id, comment_id, page_id, from_name, text, from_id")
      ...
      if (!aiGate.allowed) {
        console.warn(`[ai-reply-facebook-comments] tenant ${tenant.id} bloqueado pelo orçamento de IA (${aiGate.reason}) — ${pending.length} comentário(s) aguardam`);
      ...
      for (const comment of pending) {
        try {
          ...
          const screen = await screenInbound(supabase, {
            tenantId: tenant.id,
            platform: "facebook",
            senderId: c.from_id || `name:${c.from_name ?? "unknown"}`,
            text: c.text ?? "",
            aiConfigured: true,
          });
          if (screen.action === "filter") {
            await supabase.from("facebook_comments")
              ...
              .eq("id", comment.id);
            console.log(`[ai-reply-facebook-comments] comentário ${comment.comment_id} FILTRADO (${screen.verdict}/${screen.layer}): ${screen.reason}`);
            continue;
          }

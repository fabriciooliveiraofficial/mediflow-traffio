...
import {
    ...
    CheckCircle2, XCircle, AlertCircle, Loader2, Zap, StickyNote,
    GitMerge, Search,
} from 'lucide-react';
...
export function FollowUpTimelineDrawer({ journey, onClose }: FollowUpTimelineDrawerProps) {
    ...
    const handleClickCompleted = (apt: any) => {
        setOutcomeModal({ appointmentId: apt.id, dateLabel: `${apt.date} ${(apt.start_time || '').slice(0, 5)}`, defaultProcedure: apt?.type?.name || '' });
    };
    ...
    return (
        <>
            ...
            <RecordOutcomeModal
                isOpen={!!outcomeModal}
                appointmentId={outcomeModal?.appointmentId ?? null}
                dateLabel={outcomeModal?.dateLabel ?? ''}
                journeyId={journey.id}
                defaultProcedure={outcomeModal?.defaultProcedure}

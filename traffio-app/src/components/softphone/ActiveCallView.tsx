    ...
    if (!stream && pc) {
      ...
      if (remoteTracks.length > 0) {
        logPlatformClient({
          level: 'info',
          source: 'ActiveCallView',
          eventName: 'audio_diagnostics_fallback',
          message: 'Creating a fallback MediaStream from tracks...'
        });
      ...
      if (stream) {
        if (audioElement.srcObject !== stream) {
          logPlatformClient({
            ...
            metadata: { tracks: stream.getAudioTracks().length }
          });
          ...
          audioElement.play()
            .then(() => {
              logPlatformClient({
                level: 'info',
                source: 'ActiveCallView',
                eventName: 'audio_diagnostics_play_success',
            ...
            .catch((err: any) => {
              logPlatformClient({
                level: 'error',
                source: 'ActiveCallView',
                eventName: 'audio_diagnostics_play_error',
                message: 'play() was blocked or failed',
          ...
          if (audioElement.paused) {
            logPlatformClient({
              level: 'info',
              source: 'ActiveCallView',
              eventName: 'audio_diagnostics_unpause',
              message: 'Audio is paused. Triggering play...'
            });
            audioElement.play().catch((err: any) => {
               logPlatformClient({
                 level: 'error',
                 source: 'ActiveCallView',
                 eventName: 'audio_diagnostics_unpause_error',
                 message: 'play() retry failed',
                 metadata: { errorMsg: err.message }
               });
            });
          }
        ...
        logPlatformClient({
          level: 'info',
          source: 'ActiveCallView',
          eventName: 'audio_diagnostics_waiting',
    ...
    return () => {
      ...
      if (audioElement) {
        logPlatformClient({
          level: 'info',
          source: 'ActiveCallView',
          eventName: 'audio_diagnostics_cleanup',
          message: 'ActiveCallView unmounted. Clearing audio element.'
        });

...
export const AcceptInvitePage = () => {
  ...
  useEffect(() => {
    if (!token) {
      setErrorMsg(t('acceptInvite.errors.invalidLink'));
      setStep('error');
      return;
    }
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          setErrorMsg(t('acceptInvite.errors.notFound'));
          setStep('error');
          return;
        }

        if (data.status === 'accepted') {
          setErrorMsg(t('acceptInvite.errors.alreadyAccepted'));
          setStep('error');
          return;
        }

        if (data.status === 'revoked') {
          setErrorMsg(t('acceptInvite.errors.revoked'));
          setStep('error');
          return;
        }

        if (data.status === 'expired' || new Date(data.expires_at) < new Date()) {
          setErrorMsg(t('acceptInvite.errors.expired'));
          setStep('error');
          return;
        }
  ...
  const validate = () => {
    ...
    if (!form.full_name.trim()) {
      errs.full_name = t('acceptInvite.errors.fullNameRequired');
      ok = false;
    }
    if (form.password.length < 6) {
      errs.password = t('acceptInvite.errors.passwordTooShort');
      ok = false;
    } else if (form.password !== form.confirmPassword) {
      errs.password = t('acceptInvite.errors.passwordMismatch');
      ok = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message ?? t('acceptInvite.errors.generic'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-white flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-red-100 p-8 text-center space-y-4">
          ...
          <h2 className="text-xl font-black text-gray-900">{t('acceptInvite.invalidTitle')}</h2>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-white flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-green-100 p-8 text-center space-y-4">
          ...
          <h2 className="text-xl font-black text-gray-900">{t('acceptInvite.successTitle')}</h2>
          <p className="text-sm text-gray-500">
            {t('acceptInvite.welcomePrefix')} <strong>{(invite?.tenants as any)?.name ?? t('acceptInvite.welcomeFallback')}</strong>!<br />
  ...
  // ── Render: Formulário ────────────────────────────────────────────────────
  const roleLabels = t('acceptInvite.roleLabels', { returnObjects: true }) as Record<string, string>;
  const tenantName = (invite?.tenants as any)?.name ?? t('acceptInvite.invitedToFallback');
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-7 text-white text-center">
            ...
            <p className="text-sm font-medium opacity-80">{t('acceptInvite.invitedToPrefix')}</p>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-7 space-y-5">
            <p className="text-sm text-gray-500 text-center -mt-1">
              {t('acceptInvite.completeRegistration')}
            </p>

            {/* Email (read-only) */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('acceptInvite.email')}</label>
            ...
            {/* Nome completo */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('acceptInvite.fullName')}</label>
            ...
            {/* Senha */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('acceptInvite.password')}</label>
              <div className="relative">
            ...
            {/* Confirmar senha */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('acceptInvite.confirmPassword')}</label>
              <div className="relative">
                <Lock className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                <input
            ...
            >
              {submitting
                ? <><Loader2 className="w-4 h-4 animate-spin" /> {t('acceptInvite.creatingAccount')}</>

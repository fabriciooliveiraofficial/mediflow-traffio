...
export const ForgotPasswordPage = () => {
    ...
    const handleReset = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast('error', t('forgotPassword.errors.generic', { message: error.message }));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-ice-50 flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white rounded-[32px] shadow-xl p-8 md:p-12 border border-ice-100 animate-in fade-in zoom-in duration-500">
                ...
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-black text-graphite-900 mb-2">{t('forgotPassword.title')}</h1>
                    <p className="text-graphite-500 font-medium">{t('forgotPassword.subtitle')}</p>
                </div>
                ...
                {sent ? (
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 text-center animate-in fade-in slide-in-from-bottom-2">
                        <p className="text-emerald-800 font-bold mb-2">{t('forgotPassword.sentTitle')}</p>
                        <p className="text-sm text-emerald-600">{t('forgotPassword.sentText')}</p>
                        <button
                            onClick={() => navigate('/login')}
                    ...
                    <form onSubmit={handleReset} className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-black text-graphite-400 uppercase ml-1">{t('forgotPassword.email')}</label>
                        ...
                        >
                            {loading ? <Loader2 className="animate-spin" size={20} /> : t('forgotPassword.submit')}
                        </button>
                    </form>
                )}
            </div>
